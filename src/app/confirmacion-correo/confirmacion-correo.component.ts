...
export class ConfirmacionCorreoComponent implements OnInit {
  ...
  confirmarCorreo() {
    ...
    this.http.post<any>('http://localhost:8080/verificarCodigo', data).subscribe({
        next: response => {
            if (response && response.message && response.message.includes('Código de verificación correcto.')) {
